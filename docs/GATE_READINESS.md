...
| P1-ROWS | Enumerate every message-bearing shard/table and prove row accounting. | A current authorized full snapshot whose coverage ledger has no unhandled message-table candidate; for every supported message table, `source rows = restored rows + rejected rows`, with zero rejections and duplicate identities. | The aggregate coverage ledger accounts for all 6,542 selected tables and 9,529,301 observed rows, including 6,291 message tables. It proves 1,854,110 source messages = 1,854,110 restored + 0 rejected, with zero duplicate identities and zero unhandled candidates; the independent archive audit agrees. Because only 25/26 databases authenticate and output scope is `diagnosticSubset`, this is strong selected-set evidence rather than the required authoritative full-snapshot proof. |
| P1-TYPES | Decode every observed logical type and retain unknowns. | The same corpus reports zero unknown observed logical types and zero semantic coverage gaps; each newly observed type has a source-preserving decoder plus sanitized regression fixture. | The aggregate has zero unknown payloads. Adjacent `voipinvitemsg`, optional `voipextinfo`, and `voiplocalinfo` support closed 207 observed type-50 gaps. Two subtype `49:19` rows remain explicit semantic gaps because their values begin with closing fragments and lack an opening XML document; raw values are retained and missing structure is not synthesized. Aggregate profiling also found a quote-link bug caused by searching compressed source bytes after decode. Of 193,503 relationship references, 1 identifier was already present, 192,991 are recoverable from retained decoded XML, 511 are genuinely absent there, and 0 lack decoded-XML evidence; restoration now uses that decoded XML while preserving original provenance. The completed diagnostic archive predates that correction and the account-bound direction implementation; missing-database/diagnostic scope prevents a universe-wide type claim. |
| P1-MEDIA | Resolve every locally downloaded multimodal/file artifact. | Every attachment reference in the authorized corpus is classified as a verified owner-authorized local file/connector derivative, or explicitly missing, remote-only, expired, deleted, corrupt, or deferred; complete mode requires no deferred, ambiguous, unsafe, or unexplained state and records digest/format/path provenance. | Synthetic multimodal restoration, exact preferred-variant/state auditing, full resource-table provenance, stale/substituted-path rejection, and conversation/time-scoped local CLI/service path retrieval pass. Audit-report format 2 separately proves media-reference presence, verified local-media presence, artifact verification, decoding, and resolved media phase. The real aggregate records 235,108 unique artifact references but intentionally defers media, so none is claimed downloaded/materialized or decoded and no production replica may consume the archive; real database-to-media resolution remains unproven. |
