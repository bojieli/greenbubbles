...
Plan revision: `PLAN.md` dated 2026-08-29
...
| --- | --- | --- |
| 0 | Complete for public research development | MIT is selected; source/binary release controls and secret hygiene are enforced. |
| 0.5 | Public 0.1.1 distribution boundary decided; research/action gates remain open | Authorized disposable corpus, real-client synchronization evidence, ordinary-contact feasibility evidence, Tencent route, and qualified legal analysis. |
...
No WeChat database passphrase, per-database key, plaintext export, real
database, or restored archive is present in the repository. An existing
owner-only per-database key export and real diagnostic artifacts are confined
to ignored private temporary storage. GreenBubbles authenticated 25 of the
snapshot's 26 databases. The signed 4.1.13 build is now supported, and an
unavailable database is explicit partial coverage rather than a publication or
synchronization stop. The completed archive is still a diagnostic subset with
deferred media and no disposable scenario, so it cannot establish a full real-
corpus or real synchronization claim. Synthetic
fixtures continue to cover the replica, synchronization, retrieval, and
consumer machinery independently.
...
| --- | --- | --- | --- |
| P0-LICENSE | Select an open-source license before public release. | An explicit repository-owner license choice, a committed license file, and consistency review of dependency and distribution obligations. | **Met 2026-08-29.** The owner selected MIT; `LICENSE`, Cargo metadata, the locked inventory, generated notices, and release preflight agree. The crate remains `publish = false` because crates.io publication is separate. |
| P05-CORPUS | Confirm useful conversation and attachment data on a compatible current client and document local coverage. | An owner-authorized disposable/test-account snapshot from a signed WeChat 4.1+ build; a redacted inventory showing conversation shards and media roots; and a restoration coverage report demonstrating actual locally available text plus representative downloaded attachments. | Owner-authorized snapshots prove that local ordinary/business message shards, message-resource/media stores, contacts, sessions, and cached Moments exist. The completed GreenBubbles-only aggregate authenticates 25/26 databases and classifies all 6,543 tables/9,537,192 observed rows as 6,292 message or 251 known auxiliary tables, with zero generic/unhandled candidates. It independently audits 1,855,548 messages, 23,589 cached Moments, and 45,601 interactions (1,924,738 source records), with zero rejects, duplicate identities, or unknown payloads; two raw-retained semantic gaps remain. It also records 4,583 conversations, 42,598 participants, and 235,277 deferred artifact references. This proves useful structured content on an observed signed 4.1.13 client within the supported signed 4.1-and-later family. One unavailable database is modeled as fault-tolerant `partialDatabaseCoverage`; the account-bound selected-set rerun and independent archive audit are complete, but the evidence is non-disposable and media-deferred rather than authoritative full-corpus proof. The metadata-only attachment inventory remains 136,786 candidates (38,874,071,097 bytes) in one root and 87 (1,469,805 bytes) in the other, without traversal issues, symbolic links, or a cap hit. The gate remains open. |
| P05-NONINVASIVE | Determine whether a lawful owner-authorized acquisition route exists, with or without modifying/re-signing WeChat, process attachment, memory scanning, or reusable credential export. | A successful real-corpus restoration from a documented official portable archive, owner-supplied plaintext SQLite, an owner-supplied passphrase-through-stdin snapshot, or the gated owner-authorized `greenbubbles-acquire` capture; the redacted acquisition record must name only that route and show the client state honestly (signed, or owner re-signed for the capture path). | Real passive snapshots and storage preflight succeed without modifying or invoking WeChat, and the passive pipeline's non-invasive guarantees are unchanged. Synthetic plaintext and encrypted/WAL restoration tests pass. A current public-project survey found no documented non-invasive macOS passphrase source—only live debugger/process hooks, memory scanning, client re-signing, already supplied keys, older iTunes/iOS backups, proprietary phone-mediated backup, or a separate official bot relationship. The owner's 2026-08-27 decision lifted the previous blanket prohibition on debugger-based acquisition: the LLDB `CCKeyDerivationPBKDF` capture was validated live on the owner's own machine and account (26/26 databases HMAC-verified on the exact 4.1.12 acquisition-helper build) and is now embedded as the owner-run `greenbubbles-acquire` path (manual owner-run re-sign, page-1 HMAC proof; see `docs/PASSPHRASE_ACQUISITION.md`). GreenBubbles restores real data directly from the already exported per-database keys without invoking that helper. Passive restoration supports the signed 4.1+ family and continues around a missing per-database key with explicit partial coverage; this demonstrates a functioning private owner-authorized route for the observed corpus, while full coverage remains a separate Phase 1 claim. |
| P05-REALSYNC | Prove bootstrap and incremental synchronization on disposable data. | Redacted real-client measurements for idle, one-message, burst, edit, recall, deletion, missed hint, integrity reconciliation, and crash/restart; committed checkpoints must never lead replica state, and new persisted text must meet the 60-second p95 objective. | Synthetic benchmark/fault cases, the audited offline restore/merge/publish transaction, and the continuous monotonic archive follower now pass, including bootstrap, changed/idempotent generations, atomic state, rollback/equivocation denial, replacement-replica binding, commit-before-state crash recovery, recoverable sealed-generation quarantine, and a read-only deep replica audit over canonical hashes/projections, links, FTS, checkpoint/coverage, and sync/change history. Pre-migration backups are independently audited for schemas 1–4, including schema-4 cached-surface preservation, and a non-destructive recovery drill seals the source namespace, creates a separate encrypted current-schema candidate, backfills historical serving state, and requires the deep audit without replacing active state. Snapshot planning/acquisition plus privacy-safe restoration, publication, application, checkpoint, bound sample, and nearest-rank p50/p95 timing schemas are implemented. The composer explicitly records that source-persistence time, inter-command delay, and disposable-scenario attribution remain absent and never claims end-to-end success. A fresh real 25-set bootstrap/incremental pair independently proves exact 8-set content-change classification and 24-entry proportional copying, with 3,362 ms bootstrap and 1,996 ms immediate-incremental acquisition totals. Later diagnostic restoration confirms structured message and cached-SNS data, but it is neither a bound incremental application nor a disposable-scenario timing run. Real end-to-end 60-second-p95 samples remain absent. |
...
| P05-LEGAL-ASSESS | Assess source, binary, schema, fixture, and hosted-repository distribution separately. | Qualified-counsel analysis for each artifact class and intended jurisdiction, plus the repository owner's recorded distribution decision. | The owner decision is recorded for public MIT source and signed/notarized macOS arm64 binaries, with separate category boundaries in `DISTRIBUTION_INVENTORY.md`. Qualified-counsel analysis is still absent; 0.1.1 is explicitly a research alpha, not a legal assurance. |
...
| P05-RESPONSE | Establish a response plan for updates, complaints, takedowns, security reports, and maintainer/host exposure. | Named owner and counsel/security contacts, intake and preservation rules, immediate private-disable/release-hold procedure, supported-build revocation steps, user notice criteria, and repository/host response procedure approved by the owner. | The repository owner is the release/security owner; GitHub private vulnerability reporting, a three-business-day acknowledgement target, release holds, artifact revocation, evidence minimization, and takedown procedures are active. Counsel escalation remains external if a legal complaint requires it. |
| P05-PUBLISHABLE | Document which components and experiments may be published. | A component-by-component publication matrix incorporating the legal assessments, Tencent outcome, dependency licenses, fixture provenance, and an explicit owner release decision. | **Met for the 0.1.1 boundary.** Source, documentation, synthetic fixtures, hosted metadata, signed/notarized arm64 binaries, and aggregate research evidence are approved. Real data remains prohibited; acquisition stays owner-run and advanced; sending ships closed. |
...
| P1-ROWS | Enumerate every message-bearing shard/table and prove row accounting. | A current authorized full snapshot whose coverage ledger has no unhandled message-table candidate; for every supported message table, `source rows = restored rows + rejected rows`, with zero rejections and duplicate identities. | The aggregate coverage ledger accounts for all 6,543 selected tables and 9,537,192 observed rows, including 6,292 message tables. It proves 1,855,548 source messages = 1,855,548 restored + 0 rejected, with zero duplicate identities and zero unhandled candidates; the independent archive audit agrees. Because only 25/26 databases authenticate and output scope is `partialDatabaseCoverage`, this is strong selected-set evidence rather than the required authoritative full-snapshot proof. |
| P1-TYPES | Decode every observed logical type and retain unknowns. | The same corpus reports zero unknown observed logical types and zero semantic coverage gaps; each newly observed type has a source-preserving decoder plus sanitized regression fixture. | The aggregate has zero unknown payloads. Adjacent `voipinvitemsg`, optional `voipextinfo`, and `voiplocalinfo` support closed 207 observed type-50 gaps. Two subtype `49:19` rows remain explicit semantic gaps because their values begin with closing fragments and lack an opening XML document; raw values are retained and missing structure is not synthesized. An earlier aggregate profile also found a quote-link bug caused by searching compressed source bytes after decode. Of that earlier set's 193,503 relationship references, 1 identifier was already present, 192,991 are recoverable from retained decoded XML, 511 are genuinely absent there, and 0 lack decoded-XML evidence; restoration now uses that decoded XML while preserving original provenance. The current selected-set archive is account-bound and audited, but missing-database/partial-coverage scope prevents a universe-wide type claim. |
| P1-MEDIA | Resolve every locally downloaded multimodal/file artifact. | Every attachment reference in the authorized corpus is classified as a verified owner-authorized local file/connector derivative, or explicitly missing, remote-only, expired, deleted, corrupt, or deferred; complete mode requires no deferred, ambiguous, unsafe, or unexplained state and records digest/format/path provenance. | Synthetic multimodal restoration, exact preferred-variant/state auditing, full resource-table provenance, stale/substituted-path rejection, and conversation/time-scoped local CLI/service path retrieval pass. Audit-report format 2 separately proves media-reference presence, verified local-media presence, artifact verification, decoding, and resolved media phase. The real aggregate records 235,277 unique artifact references but intentionally defers media, so none is claimed downloaded/materialized or decoded; real database-to-media resolution remains unproven. |
...
The adapter is **implemented and shipped closed** (`SEND_ADAPTER.md`). Opening
it is still blocked until the Phase 0.5 technical, action, and
legal/supportability gates and the Phase 1 restoration exit gate pass: the four
gate-evidence flags in the send configuration stand for exactly those
decisions, the guard denies while any is false, and a build with no pinned
release verifying key cannot leave the dry-run stage regardless. The disposable
test account and peer must not be an ordinary personal contact.
...
| P4-TEXT | Start with confirmed text to one allow-listed disposable conversation. | One narrowly scoped adapter, default-off capability, immutable preview/approval, and redacted observed-sent/failed/unknown live test evidence. | The adapter exists, is default-off, and is confined by rollout stage to File Transfer and at most one reviewed peer. No live attempt has been made; redacted live evidence remains outstanding. |
| P4-APPROVAL | Bind approval to the exact immutable draft. | Tests proving that account, recipient, reply target, text, attachment, expiry, policy, or connector-version changes invalidate a one-use approval token. | **Met.** `send approve --confirm` issues owner-only, time-bounded evidence bound to the exact gate, capability, draft, account, conversation, adapter, and build; the outbox consumes approval identities durably; the deterministic idempotency key makes a second dispatch of the same approval impossible. Tests cover every binding dimension and the consumed-identity refusal. |
| P4-GUARDS | Enforce idempotency, rates, global kill switch, and version failure outside the model. | Deterministic adapter-bound tests including concurrent retries, restart recovery, rate exhaustion, kill-switch activation, and build drift, with no network/client invocation after denial. | **Met.** The outbox is single-flight, `flock`-serialized, and durable; restart recovery parks a dispatched attempt and never re-dispatches; rate exhaustion, both kill switches (local and signed-matrix), unknown build, and calibration drift each deny before the dispatcher is called, which the tests assert by counting dispatcher invocations. |
| P4-LIFECYCLE | Model drafted/approved/attempted/observed-sent/observed-failed/unknown. | Transactional state-machine tests plus live reconciliation evidence; no internal acknowledgement is labeled delivery. | The serving process now produces persistent approval/attempt/reconciliation transitions into the hash-chained journal, and `audit-connector-state` validates their per-draft ordering. `observedSent` is reachable only through replica reconciliation; a visually confirmed helper outcome parks the attempt instead. Live reconciliation evidence remains outstanding because no live attempt has been made. |
| P4-RECONCILE | Link official-client result back to replica and audit. | The live test message is independently observed in official-client/local state, deduplicated into the replica, and linked to the immutable action and audit IDs; ambiguous results remain unknown. | The mechanism exists: `send reconcile` searches the encrypted replica for an outgoing message in the bound conversation matching the normalized body digest, resolves the parked attempt, and appends a linked reconciliation event; an unmatched attempt stays unknown until the grace window elapses. Live evidence still requires a safe live text action. |
...
1. Keep every real corpus and credential private and obtain an owner-authorized
   disposable current-version corpus through the official/plaintext/passphrase
   acquisition order in `ACQUISITION_FEASIBILITY.md`.
...
6. Re-run the target-specific license, notice, signing, notarization, and
   clean-machine review before adding a new architecture or distribution.
