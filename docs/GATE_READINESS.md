...
| P05-REALSYNC | Prove bootstrap and incremental synchronization on disposable data. | Redacted real-client measurements for idle, one-message, burst, edit, recall, deletion, missed hint, integrity reconciliation, and crash/restart; committed checkpoints must never lead replica state, and new persisted text must meet the 60-second p95 objective. | Synthetic benchmark and fault cases pass. A retained real 25-set bootstrap/incremental pair independently proves exact 9-set content-change classification and 27-entry proportional copying, but contents stayed encrypted, so message-case attribution, replica publication, and latency still require passphrase-through-stdin restoration on disposable data. |
...
| P4-APPROVAL | Bind approval to the exact immutable draft. | Tests proving that account, recipient, reply target, text, attachment, expiry, policy, or connector-version changes invalidate a one-use approval token. | The offline contract binds external approval evidence to the exact gate, capability, draft, account, conversation, adapter, and build and rejects consumed IDs, but it deliberately cannot issue or consume an approval. Adapter-owned authentication and transactional one-use proof remain gated. |
...
| P4-LIFECYCLE | Model drafted/approved/attempted/observed-sent/observed-failed/unknown. | Transactional state-machine tests plus live reconciliation evidence; no internal acknowledgement is labeled delivery. | The offline model validates only monotonic drafted/approved/attempted/observed-sent/observed-failed/unknown sequences and has no delivered state. The serving process still exposes only draft/review; persistent transitions and live reconciliation remain gated. |
