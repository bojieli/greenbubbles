...
| P1-TYPES | Decode every observed logical type and retain unknowns. | The same corpus reports zero unknown observed logical types and zero semantic coverage gaps; each newly observed type has a source-preserving decoder plus sanitized regression fixture. | The aggregate has zero unknown payloads. Adjacent `voipinvitemsg`, optional `voipextinfo`, and `voiplocalinfo` support closed 207 observed type-50 gaps. Two subtype `49:19` rows remain explicit semantic gaps because their values begin with closing fragments and lack an opening XML document; raw values are retained and missing structure is not synthesized. Aggregate profiling also found a quote-link bug caused by searching compressed source bytes after decode. Of 193,503 relationship references, 1 identifier was already present, 192,991 are recoverable from retained decoded XML, 511 are genuinely absent there, and 0 lack decoded-XML evidence; restoration now uses that decoded XML while preserving original provenance. The completed diagnostic archive predates that correction, 1,389,480 message directions remain unknown without verified local-account identity, and missing-database/diagnostic scope prevents a universe-wide type claim. |
